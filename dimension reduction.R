...
# scale pcs by % variation explained
weights <- c(28, 15, 13, 12)
group_CD_scaled <- group_CD * weights
group_AB_scaled <- group_AB * weights

# find the convex hull of polygons
hull_AB <- convhulln(group_AB_scaled,output.options = TRUE,options = "FA")
hull_CD <- convhulln(group_CD_scaled,output.options = TRUE,options = "FA")
#unscale
hull_AB <- convhulln(group_AB,output.options = TRUE,options = "FA")
hull_CD <- convhulln(group_CD,output.options = TRUE,options = "FA")

(hull_CD$vol) / (hull_AB$vol)
